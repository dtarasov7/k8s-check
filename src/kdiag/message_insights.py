...
MESSAGE_INSIGHT_CATALOG = (
    ...
    _entry(
        ...
        "routine",
        r"coredns|kube-dns",
        r"kubeforward.*(?:updated servers|forward servers updated|number of endpoint\s*slices in cache|successfully updated.*endpoint\s*slice)",
    ...
    _entry(
        "coredns_startup_activity",
        "routine",
        r"coredns|kube-dns",
        r"(?:proceeding to serve|leaving gomaxprocs.*cpu quota undefined|plugin/forward.*dialing)",
        "CoreDNS сообщил о штатной работе",
        "Startup/runtime или debug-запись без самостоятельного признака ошибки DNS.",
        "Проверять только вместе с SERVFAIL, timeout, forwarding loop, неготовностью Pod или частыми рестартами.",
        "Скрывать штатную запись; при проблеме сопоставить CoreDNS readiness, DNS errors и EndpointSlice upstream-сервиса.",
    ),
    _entry(
    ...
    _entry(
        ...
        "routine",
        r"kube-rbac-proxy",
        r"(?:parsed upstream|parsing configuration from environment|using config|added upstream|ignored paths|added (?:liveness|readiness) probe|valid token audiences|generating self.?signed cert as no cert is provided|running on(?:\s|$))",
    ...
    _entry(
        "kubelet_pause_container_started",
        "routine",
        r"kubelet",
        r"start(?:ed|ing)? container pause\b",
        "kubelet запустил sandbox-контейнер",
        "Запуск pause-контейнера является штатным этапом создания Pod sandbox.",
        "Проверять только при соседнем FailedCreatePodSandBox, CNI/runtime failure или цикле перезапусков.",
        "Скрывать успешную запись; фактическое состояние Pod и sandbox проверять по CRI и объектам Kubernetes.",
    ),
    _entry(
    ...
    _entry(
        ...
        "routine",
        r"cert-manager",
        r"(?:certificate\s*request has been approved|certificate has been (?:issued successfully|successfully issued))",
    ...
    _entry(
        ...
        "routine",
        r"stale-dns-connections-cleaner",
        r"(?:then every .* seconds|checks interval.*timeout)",
    ...
    _entry(
        "stale_dns_cleaner_connection_count",
        "routine",
        r"stale-dns-connections-cleaner",
        r"established connection count",
        "DNS cleaner посчитал соединения",
        "Периодическая telemetry-запись отражает число просмотренных соединений и сама по себе не является ошибкой.",
        "Проверять только при аномальном объёме вместе с DNS/CNI failures.",
        "Скрывать как routine telemetry; при сетевой деградации сопоставить с conntrack, CoreDNS и Cilium.",
    ),
    _entry(
    ...
    _entry(
        "control_plane_startup_activity",
        "routine",
        r"control-plane-manager",
        r"(?:podcidr on node .* is |kubeclient successful init)",
        "Control-plane manager сообщил о штатной инициализации",
        "Компонент получил PodCIDR либо успешно инициализировал Kubernetes client; признака отказа в строке нет.",
        "Проверять только при неготовом control-plane Pod, API readyz failure или устойчивом reconciliation churn.",
        "Скрывать штатную запись; состояние control plane проверять по Pod, Node и API readyz.",
    ),
    _entry(
        "control_plane_ignored_kubeadm_document",
        "observe",
        r"control-plane-manager",
        r"ignor(?:ed|ing).*yaml document.*groupversionkind kubeadm\.k8s\.io/.*kind[=: ]+joinconfiguration",
        "Control-plane manager пропустил kubeadm JoinConfiguration",
        "Парсер искал другой kubeadm document и пропустил JoinConfiguration. В смешанном YAML это может быть ожидаемо, но устойчивое повторение при незавершённом rollout требует проверки совместимости формата.",
        "Проверять при неготовом control plane, повторяющемся rollout или соседней ошибке чтения kubeadm configuration.",
        "Сопоставить версию Kubernetes/Deckhouse, последующие сообщения о применённой конфигурации и готовность control-plane Pod; файл автоматически не изменять.",
    ),
    _entry(
    ...
    _entry(
        ...
        "actionable",
        r"kube-apiserver|apiserver",
        r"authentication config file.*(?:no such file|permission denied|is a directory)|(?:failed|unable) to read authentication config file",
    ...
    _entry(
        "go_runtime_stack_trace",
        "actionable",
        r".+",
        r"(?:runtime/debug\.stack|goroutine [0-9]+ \[running\])",
        "Компонент записал Go stack trace",
        "Stack trace показывает место формирования диагностического дампа, но без предшествующей строки не доказывает panic и не указывает первопричину.",
        "Расследовать при panic/fatal рядом по времени, рестартах, неготовности Pod или повторении дампа.",
        "Открыть исходный лог перед stack trace, определить trigger и первую ошибку, затем сопоставить restartCount, lastState и readiness Pod; не использовать последнюю строку стека как диагноз.",
    ),
    _entry(
