...
`normalized-events.json.gz` содержит дедуплицированные распознанные события, независимые эпизоды совпадений по времени, счётчики усечения, автономные карточки известных сообщений и ограниченный список частых нераспознанных шаблонов. Карточки используют только уже собранные данные; LLM, сеть и внешние API не нужны. Штатные сообщения и сообщения для наблюдения остаются только в этом конфиденциальном машинном файле, кроме случаев, когда локальное сопоставление обнаружило нездоровый связанный Pod или другой явный признак проблемы; stack traces и другие требующие внимания сообщения выводятся с объяснением, условием реакции и рекомендацией. Неизвестные шаблоны без проверенной интерпретации больше не печатаются списком в основном Markdown: там остаётся только их число и ссылка на машинный файл. Передавать файл за пределы контура без обезличивания нельзя.
