...
`normalized-events.json.gz` содержит дедуплицированные распознанные события, независимые эпизоды совпадений по времени, счётчики усечения, автономные карточки известных сообщений и ограниченный список частых нераспознанных шаблонов. Карточки используют только уже собранные данные; LLM, сеть и внешние API не нужны. Markdown кратко показывает уже проанализированные штатные/наблюдаемые сообщения с операторским выводом и условием действия; stack traces и другие требующие внимания сообщения получают подробную карточку. Для прозрачности выводятся не более пяти действительно ещё не классифицированных шаблонов из разных компонентов с общей безопасной рекомендацией; полный набор остаётся в конфиденциальном машинном файле. Передавать его за пределы контура без обезличивания нельзя.
