...
- Markdown теперь показывает компактную таблицу уже проанализированных штатных/наблюдаемых сообщений: смысл, частоту и условие действия. Поэтому известные строки не исчезают молча и не выглядят как ошибки.
- Для ручной классификации остаётся не более пяти действительно неизвестных шаблонов из разных компонентов. Они явно не считаются findings; полный набор и примеры сохраняются в `normalized-events.json.gz`, а повторный `report` применяет новые правила без повторного snapshot.

## [0.11.1] - 2026-08-28

### Исправлено

- Проекция `cilium-dbg service list --output json` читает фактические frontends Cilium 1.17 из `status.realized`. Неизвестный, частичный или усечённый JSON теперь блокирует сравнение, поэтому непустая карта больше не превращается в ложное отсутствие всех ClusterIP.
- Finding Cilium ограничен счётчиком и несколькими примерами вместо повторения полного списка Service для каждого узла; старые коллекции с пустыми frontend-проекциями также обрабатываются fail-closed.
- Штатные сообщения Deckhouse со startup, успешной выдачей сертификатов, kubeforward, kubeclient/PodCIDR и DNS-cleaner классифицируются автономно. Go stack trace получает отдельное объяснение и рекомендации по проверке контекста и состояния Pod.
- Неизвестные шаблоны без проверенной интерпретации больше не печатаются полотном в Markdown; полный bounded-набор остаётся в `normalized-events.json.gz`.

## [0.11.0] - 2026-08-27

### Добавлено

- Отдельный workflow baseline: создание кандидата из проверенной collection, явное утверждение с автором и source-aware сравнение новых коллекций.
- SHA-256 устойчивого профиля и всего канонического утверждённого документа; изменение профиля, approval-метаданных или байтового представления приводит к отказу проверки.
- Классы отличий `new_problem`, `removed`, `added`, `changed`, `resolved`, `unverifiable`, машиночитаемый `baseline-comparison.json` и русскоязычный `baseline-comparison.md` с рекомендуемыми действиями.
- Параметр `snapshot --baseline`, использующий тот же модуль сравнения и включающий результаты в manifest collection.

### Безопасность

- Approval блокируется при активных critical findings и существенных пробелах обязательных источников; исключение требует явного `--override-unsafe` и сохраняет причины.
- Утверждённый baseline не перезаписывается, кандидат не принимается для сравнения, а отсутствующий источник новой collection никогда не превращает его объекты в ложные `removed`.
- Из профиля исключены timestamps, UID, IP/PID, Lease times, отдельные logs, динамические Job/ReplicaSet и случайные Pod suffix. Внешнее хранение и автоматическое обучение нормы отсутствуют.

## [0.10.0] - 2026-08-27

### Добавлено

- Режимы `--purpose check|incident` с обязательным явным окном инцидента через `--incident-since` или абсолютные start/end. Окно применяется к journald, Kubernetes Pod logs и Prometheus.
- Состояния проблем `active`, `resolved`, `unknown` и роли `possible_cause`, `consequence`, `configuration_risk`; обычный отчёт скрывает завершившиеся исторические проблемы, сохраняя их в JSON.
- Шесть фиксированных ограниченных Prometheus `query_range` для API, etcd, рестартов, сети и CPU в режиме incident.
- Ограниченный топологический причинный граф Kubernetes/storage/infrastructure, связи «может объяснять» и детерминированный рейтинг возможных причин.
- Файл `causal-graph.json`; ранжирование и диапазонные сигналы включены в локальный и псевдонимизированный LLM-пакеты.

### Изменено

- Отчёт явно показывает назначение и окно запуска, русские состояния/роли, ранжирование, сводку графа и изменения метрик. Балл гипотезы описан как порядок проверки, а не вероятность.
- Тип нового collection изменён на `diagnostic_collection`; schema_version остаётся 1.
- Нормализованные записи с точным временем вне окна incident исключаются до правил и карточек сообщений; raw Kubernetes Events/Pod logs при этом не переписываются. Rule pack: `2026.08.14`.

## [0.9.2] - 2026-08-27

### Изменено

- Убран прямой запуск `crio --version`: CRI-O по-прежнему распознаётся как активный runtime по `crio.service` и данным CRI, но отсутствие необязательного host-бинарника больше не создаёт шум в полноте сбора.
- Штатные success/info-записи kubelet, StatefulSet, kube-apiserver policy refresh, cert-manager, kube-rbac-proxy, CoreDNS kubeforward и control-plane-manager со скриншота классифицируются локально и скрываются. Они выводятся оператору только при связанной неисправности либо при гарантированном аномальном объёме: не менее 1000 записей или не менее 100 записей при нижней оценке 100 записей/ч.
- Исторические ошибки чтения Deckhouse authentication config больше не выводятся, если файл сейчас существует и читается, API `readyz` успешен, а все собранные Pod kube-apiserver готовы.

### Исправлено

- Если `crictl exec` не работает или CRI-инвентарь недоступен, `cilium-dbg`/`cilium` ищется только в стандартных путях rootfs точного работающего процесса `cilium-agent`; успешный read-only запуск заменяет ошибки host-проверок.
- Поиск `etcdctl` понимает PID как объект и как JSON-строку из `crictl inspect`, а при недоступном CRI использует стандартные пути rootfs точного работающего процесса `etcd`.
- Deckhouse DNS discovery сначала проверяет фактический ConfigMap `kube-system/d8-kube-dns`, сохраняя прежние Deckhouse и vanilla варианты как fallback.

## [0.9.1] - 2026-08-27

### Исправлено

- Cilium discovery сопоставляет CRI containers с Pod sandbox и распознаёт vanilla Pod `kube-system/cilium-*` и Deckhouse Pod `d8-cni-cilium/agent-*` с контейнером `cilium-agent`. Внутри контейнера проверяются имена и стандартные абсолютные пути `cilium`, `cilium-dbg` и `cilium-debug`; успешный fallback больше не оставляет в отчёте эквивалентные ошибки отсутствующих host-бинарников.
- Если `crictl exec` в контейнер etcd недоступен, read-only проверки повторяются бинарником из rootfs этого же работающего контейнера через проверенный PID `/proc/<pid>/root`. Host `etcdctl` в `PATH` сохранён как fallback; небезопасный общий поиск бинарников по слоям произвольных образов не используется.
