...
`normalized-events.json.gz` contains deduplicated categorized events, independent scoped correlation episodes, explicit truncation/drop counters by source, offline message-insight cards, and bounded approximate heavy hitters for unknown fingerprints. Recognized cards explain common messages and correlate only data already present in the snapshot; they are not findings and do not use an LLM, network, or external API. Routine/observe cards stay in this machine-readable confidential file unless local correlation detects an unhealthy related Pod or another explicit problem; stack traces and other actionable/security cards without a duplicate finding enter the operator report with an explanation and recommendation. Unknown templates without a verified interpretation are no longer printed as a list in the main Markdown; it retains only their count and the machine-file reference. Original messages remain confidential; do not transfer this file outside the trusted environment without a separate redaction review.
