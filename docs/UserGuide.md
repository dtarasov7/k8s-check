...
The embedded catalogue still classifies recognized templates as routine, observe, actionable, or security. Successful kubelet, StatefulSet, kube-apiserver policy-refresh, cert-manager, kube-rbac-proxy, CoreDNS kubeforward, DNS-cleaner, and control-plane-manager messages remain only in confidential `normalized-events.json.gz`. A routine message is shown only with a correlated failure or guaranteed abnormal volume: at least 1,000 records, or at least 100 records with a lower-bound rate of 100 records/hour. The estimated upper bound is never used for this decision. Actionable/security entries without a duplicate deterministic finding are also shown. No LLM, network, or external API is used.
