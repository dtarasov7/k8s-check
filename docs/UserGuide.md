...
The collector first uses crictl to invoke etcdctl inside the already running local etcd container, which matches Deckhouse static Pods. If container exec fails, it accepts both object and JSON-string PID forms from `crictl inspect`. If CRI is unavailable, only the exact running `etcd` process is selected before using a standard etcdctl path below `/proc/<pid>/root`; a host etcdctl remains another fallback. An arbitrary binary from an unrelated containerd snapshot is never executed. External etcd and non-standard layouts produce an evidence-gap result rather than an invented diagnosis.

For Cilium, host `cilium`, `cilium-dbg`, and `cilium-debug` commands are tried first. If they are unavailable, the same bounded read-only status and service-list commands are run through crictl. CRI discovery recognizes `cilium-*` in `kube-system` for Cilium 1.14 and `agent-*` in `d8-cni-cilium` with the `cilium-agent` container for Deckhouse/Cilium 1.17; both CLI names and standard absolute paths are tried inside the container. If exec is rejected or CRI inventory is unavailable, the CLI is resolved only from standard paths in the rootfs of the exact `cilium-agent` process. Cilium 1.17 JSON is read from its actual `status.realized` layout; an unknown, partial, or truncated shape blocks service-map comparison instead of becoming an empty map. Kubernetes `pods/exec` permission is not required.

CRI-O is detected through `crio.service` and CRI evidence. The collector does not run a separate host `crio --version` probe or report a missing optional binary.
...
The enabled collector reads Nodes, Pods, Events, workloads, Services, EndpointSlices, PDBs, PVCs, PVs, APIService, Leases, VolumeAttachments, CSI objects, StorageClasses, NetworkPolicies, selected Cilium CRDs, allowlisted DNS/Cilium ConfigMap fields, the non-resource URL <code>/readyz</code>, and Pod logs only in approved namespaces. ConfigMap discovery first tries Deckhouse `kube-system/d8-kube-dns`; it also recognizes `d8-cni-cilium/cilium-configmap`, legacy `d8-kube-dns/d8-kube-dns`, `d8-kube-dns/node-local-dns`, and vanilla `cilium-config`, `coredns`, and `node-local-dns` names while recording every attempt and selected object.
...
The embedded catalogue classifies recognized templates as routine, observe, actionable, or security. Successful kubelet, StatefulSet, kube-apiserver policy-refresh, cert-manager, kube-rbac-proxy, CoreDNS, DNS-cleaner, and control-plane-manager messages remain only in confidential `normalized-events.json.gz`. This includes common startup/telemetry messages: pause-container startup, successful certificate issuance, kubeforward updates, kube-rbac-proxy self-signed startup certificate, PodCIDR/kubeclient initialization, and DNS-cleaner counters. A routine message is shown only with a correlated failure or guaranteed abnormal volume: at least 1,000 records, or at least 100 records with a lower-bound rate of 100 records/hour. A Go stack trace is not declared a cause, but is shown as an actionable card recommending inspection of the preceding error, restartCount/lastState, and Pod readiness. No LLM, network, or external API is used.

For Deckhouse authentication config, node collection records only file metadata for `/etc/kubernetes/deckhouse/extra-files/authentication-config.yaml`, never its contents. A single read error during restart/reconciliation is treated as a normal transient race and omitted from findings. Historical repeated records are also omitted when the file is currently present and readable, API readyz passes, and every collected kube-apiserver Pod is ready. If any of those conditions is not confirmed, the repeated error remains a warning; visibility inside the container mount namespace is not directly verified.
...
| <code>controlplane.authentication_config_read_error</code> | fact | kube-apiserver repeatedly reports that its configured authentication file cannot be read, while current healthy file, readyz, and Pod states are not all confirmed. | Check current host metadata, readyz, Pod readiness, mount/path, permissions, and the Deckhouse reconciliation window; do not create an empty replacement file. |
