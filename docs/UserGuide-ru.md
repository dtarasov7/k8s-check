...
Неизвестные fingerprints без проверенной семантики не получают выдуманную рекомендацию и не печатаются строка за строкой в основном Markdown. Отчёт показывает только число сохранённых неизвестных шаблонов и путь к `normalized-events.json.gz`; полный bounded-набор, примеры и приблизительная частота остаются в этом конфиденциальном машинном файле для последующего анализа и расширения локального каталога.
