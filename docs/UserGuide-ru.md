...
Сначала через crictl вызывается etcdctl внутри уже работающего локального контейнера etcd, что соответствует static Pod Deckhouse. Если container exec не работает, PID извлекается из объектной или строковой формы `crictl inspect`. При недоступном CRI выбирается только точный работающий процесс `etcd`, после чего используется стандартный путь `etcdctl` в `/proc/<pid>/root`; host etcdctl остаётся ещё одним fallback. Произвольный бинарник из несвязанного containerd snapshot не запускается. Для внешнего etcd и нестандартной раскладки будет сформировано сообщение о недоступной проверке, а не недостоверный диагноз.

Для Cilium сначала проверяются host-команды `cilium`, `cilium-dbg` и `cilium-debug`. Если их нет, те же ограниченные read-only команды status/service list запускаются через crictl. CRI discovery распознаёт `cilium-*` в `kube-system` для Cilium 1.14 и `agent-*` в `d8-cni-cilium` с контейнером `cilium-agent` для Deckhouse/Cilium 1.17; внутри контейнера проверяются имена и стандартные абсолютные пути CLI. При запрещённом exec или недоступном CRI бинарник ищется только в стандартных путях rootfs точного процесса `cilium-agent`. JSON Cilium 1.17 читается из фактического `status.realized`; неизвестный, частичный или усечённый формат блокирует сравнение Service map, а не превращается в пустую карту. Право Kubernetes `pods/exec` не требуется.

CRI-O определяется по `crio.service` и данным CRI. Отдельный host-запуск `crio --version` не выполняется и не создаёт сообщение об отсутствующем бинарнике.
...
Встроенный каталог классифицирует известные сообщения как штатные, требующие наблюдения, требующие внимания или относящиеся к безопасности. Успешные записи kubelet, StatefulSet, kube-apiserver policy refresh, cert-manager, kube-rbac-proxy, CoreDNS, DNS cleaner и control-plane-manager остаются только в конфиденциальном `normalized-events.json.gz`. В том числе распознаются характерные startup/telemetry-сообщения: pause container, успешная выдача сертификата, kubeforward, self-signed startup cert kube-rbac-proxy, PodCIDR/kubeclient init и счётчики DNS cleaner. Штатное сообщение выводится оператору лишь при связанной неисправности или гарантированном аномальном объёме: не менее 1000 записей либо не менее 100 записей при нижней оценке 100 записей/ч. Go stack trace не объявляется причиной, но выводится как требующая внимания карточка с проверкой предыдущей ошибки, restartCount/lastState и readiness Pod. LLM, сеть и внешние API не используются.

Для Deckhouse authentication config на узле собираются только метаданные файла `/etc/kubernetes/deckhouse/extra-files/authentication-config.yaml`, но не содержимое. Одиночная ошибка чтения при restart/reconciliation считается нормальной кратковременной гонкой и не выводится. Исторические повторные записи также скрываются, если файл сейчас существует и читается, API readyz успешен и все собранные Pod kube-apiserver готовы. При невыполнении хотя бы одного из этих условий повторная ошибка остаётся предупреждением; видимость внутри mount namespace контейнера напрямую не проверяется.
...
Уже классифицированные штатные и наблюдаемые fingerprints выводятся компактной таблицей: компонент, смысл, частота и условие, при котором требуется действие. Они больше не выглядят как неизвестные ошибки. Для действительно неизвестных fingerprints программа не придумывает семантику: Markdown показывает максимум пять наиболее частых шаблонов из разных компонентов как очередь ручной классификации и даёт общую рекомендацию проверить исходный контекст только при совпадении с активной деградацией. Полный bounded-набор, примеры и приблизительная частота остаются в конфиденциальном `normalized-events.json.gz`; после добавления правила повторная команда `report` переводит совпавший шаблон в проанализированную карточку без нового snapshot.
...
| <code>controlplane.authentication_config_read_error</code> | fact | kube-apiserver повторно сообщает, что настроенный authentication file нельзя прочитать, и текущее исправное состояние файла, readyz и Pod одновременно не подтверждено. | Проверить текущие метаданные host-файла, readyz, готовность Pod, mount/path, права и окно Deckhouse reconciliation; не создавать пустой файл вместо отсутствующего. |
